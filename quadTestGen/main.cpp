...
int main() {
    ...
    for(int i = 1; i < 207; i++){ //beyond 33 will exceed the range todo extract when finished
        std::string fileName = "/Users/willdunn/WillDunn/quadTestGen/tests/ASCII" + std::to_string(i) + ".txt";
        fileToWriteTo.open(fileName);
        if(!fileToWriteTo) exit(0);
        std::string asciiStr;
        hasInvalidChar = false;
        for(int j = 0; j < 7; j++) { //fill in six random ASCII characters
    ...
    for(int i = 1; i < 17; i++){ //anything greater than 16 will exceed constraints todo extract when finished
        ...
        << std::to_string(6*i) << " " << std::to_string(6*i) << " " << std::to_string(2*i) << " " << std::to_string(4*i);
        fileToWriteTo.close();
    }


    //make squares
    for(int i = 1; i < 100; i++){ //100 is the max number of squares that can be made with our constraints todo extract when finished
